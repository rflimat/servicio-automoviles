...
class ComprobanteController extends Controller
{
    ...
    {
        // falta
        $comprobante = Comprobante::select('comprobantes.id','nro_comprobante','comprobantes.fecha_hora_creacion','comprobantes.fecha_hora_cancelacion','idServicio',
        'servicio','idMetodo_pago','metodo','comprobantes.estado','costo_total','idVenta','idTrabajo')
            ->join('tipos_servicio','idServicio','=','tipos_servicio.id')
            ->join('metodos_pago','idMetodo_pago','=','metodos_pago.id')
        ...
        if($comprobante->idServicio == 2 or $comprobante->idServicio == 3){
            // caso en que sea venta o venta + trabajo
            // se busca al cliente por la venta
            $comprobante['cliente'] = Venta::selectRaw('CONCAT(clientes.Nombres, " ", clientes.Apellidos) as cliente')
            ->join('clientes','clientes.id','=','idCliente')
            ->where('ventas.id',$comprobante->idVenta)
            ->first()->cliente;
            // si es caso 3 entonces tambien incluye una placa
            if($comprobante->idServicio == 3){
                $comprobante['placa'] = Trabajo::join('vehiculos','vehiculos.id','=','idVehiculo')
                    ->where('trabajos.id',$comprobante->idTrabajo)
                    ->first()->placa;
            }
        }else{
            // caso que solo sea trabajo y no venta
            $comprobante['placa'] = Trabajo::join('vehiculos','vehiculos.id','=','idVehiculo')
                ->where('trabajos.id',$comprobante->idTrabajo)
                ->first()->placa;
            $comprobante['cliente'] = Trabajo::selectRaw('CONCAT(clientes.Nombres, " ", clientes.Apellidos) as cliente')
                ->join('vehiculos','vehiculos.id','=','idVehiculo')
                ->join('clientes','clientes.id','=','cliente_id')
                ->where('trabajos.id',$comprobante->idTrabajo)
                ->first()->cliente;
        ...
        $comprobante['productosVenta'] = DetalleVenta::select('detalle_ventas.id','nombre','detalle_ventas.cantidad','precio_venta','importe')
            ->join('productos','productos.id','=','idProducto')
            ->where('idVenta',$comprobante->idVenta)
            ->get();
        $comprobante['detalleTrabajo'] = DetalleTrabajo::select('id','descripcion','fecha_hora','costo')
                ->where('idTrabajo',$comprobante->idTrabajo)->get();
