...
const Edit = () => {
    ...
    useEffect(() => {
        const getById = async () => {
            const data = await get(`${import.meta.env.VITE_API_URL}/clientes/${id}`);
            setElement(data);
        }
        getById();
    }, []);
    ...
    const validationType = useFormik({
        ...
        onSubmit: (element) => {
          editSwal("clientes").then((result) => {
            if (result.isConfirmed) {
              put(`${import.meta.env.VITE_API_URL}/clientes/${id}`, element)
                .then((res) => {
                  successSwal("cliente", "actualizado").then(() => {
                    navigate("/clientes");
                  });
                })
                .catch((err) => {
